...
int XmlDec_Svg (OSCTXT* pctxt, Svg* pvalue)
{
   int stat = 0;
...
int XmlDec_Svg_PDU (OSCTXT* pctxt, Svg* pvalue)
{
   int stat;
   ...
   stat = rtXmlpMatchStartTag (pctxt, OSUTF8("svg"), 0);
   if (stat < 0) return LOG_RTERR (pctxt, stat);

   stat = XmlDec_Svg (pctxt, pvalue);
   if (stat != 0) return LOG_RTERR (pctxt, stat);
